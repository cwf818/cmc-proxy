...
每次请求完成时，在 `RES` 行末尾追加**最近 1 / 10 / 50 次请求**的缓存命中率与速度：

```
[20:16:30.000] RES 200 POST /v1/messages took=2.35s out=1736B in:1234 out:567 rt:480 cr:890 cw:0 ch:56%,98%,99% ts:33/s,40/s,50/s
```

- 值个数随历史请求数变化：**1 次显示 1 值 → 2–10 次显示 2 值 → ≥11 次显示 3 值**
- 每个值对应滚动窗口：第 1 个 = 最近 1 次，第 2 个 = 最近 10 次，第 3 个 = 最近 50 次

| 字段 | 含义 |
|---|---|
| `ch` | 缓存命中率 = `cr ÷ (in + cr)`，**波段色**：<60 红 / 60–79 橙 / 80–89 黄 / 90–94 绿 / 95–97 亮绿 / ≥98 亮青 |
| `ts` | 生成速度（tokens/s），**波段色**：<20 红 / 20–39 橙 / 40–59 黄 / 60–79 绿 / ≥80 亮绿 |

> 波段色按逗号分段：逗号跟随其后的数值一起着色（如 `ts:33/s`、`,40/s`、`,50/s` 各自独立着色）。统计为内存态，进程重启后清零。
