...
7. **全部试完**：把**最后一次**尝试的上游响应（状态码 + body）或错误透传给客户端，客户端看到的是真实收尾结果；轮换过程打印 `上游 <status> (<model>), 轮换 → <next> (i/N): <错误体摘要>`，网络层错误打印 `上游请求失败 (<model>): code=<错误码> msg=<错误信息>, 轮换 → <next> (i/N)`。错误码/信息来自 `e.cause` 链（Node fetch 只抛 `fetch failed`，真正原因是 `ECONNREFUSED` / `ENOTFOUND` / `UND_ERR_CONNECT_TIMEOUT` 等），首字节超时/客户端断开分别标注 `FIRST_BYTE_TIMEOUT` / `CLIENT_ABORT`；502 收尾与客户端错误体同样带 `code=` / `msg=` 摘要，便于定位。
