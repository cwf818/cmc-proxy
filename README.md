...
[20:16:00.510]@S3#2 200 POST /v1/messages took=1.69s out=567B qwait:1.2s in:987 out:45 cr:0 cw:0 ch:40% peak^ cost=$0.026786 credit=0.031250 gap:10 ts:26.6/s
...
| `peak^` / `cost:` / `credit:` | **单次请求成本与额度消耗**，**位于 `ch:` 之后、`cost:` 在前**。`cost:`（橙色）为美元成本（按模型目录 `priceUsdPerMTok` 牌价直接计算，6 位小数）；`credit:`（黄色）为额度 = `cost` × `plan.credits` ÷ 模型 `monthlyCredits`（6 位小数）。**两者不保持固定比例**：不同模型 `monthlyCredits` 不同。仅当配置 `modelCatalog` 且模型已收录、本次解析到 usage 时输出。**`peak^`（红色）表示当前 UTC 时刻处于该模型 `offPeak.windows` 高峰窗口**（已按 `peakUsdPerMTok` 覆盖 input/output/cacheRead 牌价；高峰仅工作日生效——周一~周五按 UTC 计，周末整天按错峰价），插在 `cost:` 之前                                                                                                                                                                                                    |
