...
| 字段                         | 含义                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `REQ` / `RES`                | 本地请求到达 / 外部返回完成。`REQ*`（红色 `*`）表示该请求到达时同会话已有在途请求，正在串行队列中等待发送上游。行首时间戳默认暗灰，**高峰时段改暗红**（亮度同暗灰，按该请求实际转发的模型是否在其 `offPeak.windows` 内判定，见 `^cost:` / `^credit:`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `^cost:` / `^credit:` / `cost:` / `credit:` | **单次请求成本与额度消耗**，**位于 `ch:` 之后、`cost:` 在前**。`cost:`（橙色）为美元成本（按模型目录 `priceUsdPerMTok` 牌价直接计算，6 位小数）；`credit:`（黄色）为额度 = `cost` × `plan.credits` ÷ 模型 `monthlyCredits`（6 位小数）。**两者不保持固定比例**：不同模型 `monthlyCredits` 不同。仅当配置 `modelCatalog` 且模型已收录、本次解析到 usage 时输出。**`^cost:` / `^credit:`（前缀带 `^`）表示当前 UTC 时刻处于该模型 `offPeak.windows` 高峰窗口**（已按 `peakUsdPerMTok` 覆盖 input/output/cacheRead 牌价；高峰仅工作日生效——周一~周五按 UTC 计，周末整天按错峰价）。**同一判定也作用于 REQ/RES 行首时间戳：高峰时该行时间戳由暗灰改暗红**，可扫一眼日志行首即知该请求是否按高峰价计费                                                                                                                                                                                                    |
